...
description: >-
  ...
  /apply-worktree or asks to apply/merge the worktree back into the main
  workspace.
disable-model-invocation: true
---
# Apply Worktree

Use only when the user explicitly invokes `/apply-worktree` (or clearly asks to apply this worktree into the main checkout).

## Goal

Commit any pending work on this worktree branch first, then merge that branch into the **main checkout** (parent branch), verify with the project quality gate, and commit any remaining parent-side fixes. Do **not** delete the worktree (that is `/delete-worktree`). Do **not** push unless the user asks.
...
2. **Commit worktree changes first (required if dirty)**
   ...
   - Follow the user’s git safety rules and commit style: review `git status` / `git diff` / recent `git log`; stage relevant paths only (never secrets such as `Secrets.xcconfig`); no `--no-verify`, no amend unless the usual amend conditions are met, no force-push, no git config changes, no `-i`.
...
5. **Quality gate (required)**
   ...
   - Prefer repo docs / `AGENTS.md` / `README.md`. Typical pattern for this iOS repo:
     1. **Core package tests** (fast, no simulator):
        ```bash
        cd GamesLibraryCore && swift test
        ```
     2. **Xcode build + tests** via `xcode-tools` MCP (preferred when available):
        - `XcodeOpenWorkspace` on `GamesLibrary.xcodeproj` (note the returned `workspaceIdentifier`).
        - `RunAllTests` with that identifier (uses `GamesLibrary.xctestplan`: unit + UI tests).
        - If tests fail to launch, try `BuildProject` with `buildForTesting: true` first, then re-run `RunAllTests`.
     3. **Fallback shell** (when MCP is unavailable):
        ```bash
        xcodebuild test \
          -project GamesLibrary.xcodeproj \
          -scheme GamesLibrary \
...
7. **Hand off**
   ...
   - Reminder: use `/delete-worktree` separately when they want the isolated checkout removed.
